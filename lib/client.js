...
// /deepseek-balance/settings (the plugin row config in the profile's
// cordis.patch.yml, written back there by the host through POST so the config
// HMR restarts the plugin live); the readout re-polls every 60 s and listens
// for the 'deepseek-balance:settings-changed' event after a save.
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var en = {
      ...
      configSource: 'The defaults come from this plugin row in the profile cordis.patch.yml; edits here are saved back into that row (cordis.patch.yml) and apply live via config HMR — no restart needed. Comments and other rows in the file are preserved.',
    ...
    var zh = {
      ...
      configSource: '默认值来自 profile 的 cordis.patch.yml 中本插件所在行;在此修改会保存回该行(cordis.patch.yml)并通过配置 HMR 实时生效,无需重启。文件中的注释与其他行都会被保留。',
    ...
    function apply(ctx) {
      ...
      function BalanceMonitorCard(props) {
        ...
        // Load the effective config (host: the plugin row in cordis.patch.yml)
      ...
      // Settings → Plugins → dedicated tab hosting the editable Balance
      // Monitor card. The section's "configurable" tab only dispatches cards
      // whose key is a settings namespace the Host serves; this plugin edits
      // its own config through the host HTTP routes (the profile's
      // cordis.patch.yml row), so it contributes its own feature-owned tab
      // instead (settings.plugins.tab is a root list slot open to any
      // registrant — no Host namespace needed).
        ...
        return slots.register(
          {
            ...
            id: 'deepseek-balance',
            order: 30,
            label: function () { return ctx.locale.bind('deepseek-balance')('cardTitle'); },
            locale: 'deepseek-balance',
          },
          function (props) {
            var t = props.t || ctx.locale.bind('deepseek-balance');
