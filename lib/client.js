...
// /deepseek-balance/settings, the host's thin proxy over the official
// ctx.settings seam (user layer persisted in $DSH_HOME/settings.yaml; saves
// carry the read revision for optimistic concurrency — a 409 re-loads the
// latest values). The card applies the save/reset response immediately; the
// readout re-polls every 60 s and the open card every 30 s as fallback.
window.__ModuleLoader__.load({
  id: '@choi-p/dsh-deepseek-balance',
  factory: (require) => {
    var module = { exports: {} };
    ...
    var en = {
      ...
      conflictStatus: 'Settings were changed elsewhere; the latest values have been loaded.',
      configSource: 'The defaults come from this plugin row in the profile cordis.patch.yml; edits here are saved to the user settings file (settings.yaml) via the dsh settings service and apply immediately — no restart needed.',
      syntaxErrorMSG: 'Check cordis.patch.yml configuration'
    };
    var zh = {
      cardTitle: '余额监控',
      ...
      conflictStatus: '配置已在别处修改,已加载最新值。',
      configSource: '默认值来自 profile 的 cordis.patch.yml 中本插件所在行;在此修改会通过 dsh settings 服务保存到用户设置文件(settings.yaml)并立即生效,无需重启。',
      syntaxErrorMSG: '检查 cordis.patch.yml 配置'
    };
    ...
    function apply(ctx) {
      ...
      // Stable id on the readout root so a stylesheet rule can target it.
      var READOUT_ID = 'dsb-balance-readout';

      // Force padding-left: 6px. Inline styles lose to any stylesheet rule
      // carrying `!important`, so the padding is also enforced through an
      // injected `!important` rule scoped to the readout id.
      ctx.effect(function () {
        var style = document.createElement('style');
        // Enforce padding with !important so any host stylesheet rule cannot
        // override the readout's layout.
        style.textContent = '#' + READOUT_ID + '{ padding-left: 6px !important; }';
        document.head.appendChild(style);
        return function () {
          if (style.parentNode) style.parentNode.removeChild(style);
        };
      }, 'deepseek-balance: readout padding');
      ...
      function BalanceDisplay(props) {
          ...
          async function doFetch() {
            try {
              ...
              if (latestBalancesRef.current.length === 0) {
                console.error('deepseek-balance: fetch failed:', err);
                if (err instanceof SyntaxError) {
                  setError(t('syntaxErrorMSG'));
                }
        ...
        if (loading) {
          return react.createElement('div', { id: READOUT_ID, style: Object.assign({}, baseStyle, { color: 'var(--dsw-alias-label-tertiary, #888)' }) }, '...');
        }
        if (error) {
          var errMsg = error.length > 60 ? error.slice(0, 60) + '...' : error;
          return react.createElement('div', { id: READOUT_ID, title: error, style: Object.assign({}, baseStyle, { color: 'var(--dsw-alias-state-error-primary, #e53e3e)', opacity: 0.8 }) }, errMsg);
        }
        ...
        if (shown.length === 0) {
          return react.createElement('div', { id: READOUT_ID, style: Object.assign({}, baseStyle, { color: 'var(--dsw-alias-label-tertiary, #888)', paddingLeft: 6 }) }, '账户余额: --');
        }
        ...
        return react.createElement('div', { id: READOUT_ID, style: baseStyle }, children);
      }
      ...
      function BalanceMonitorCard(props) {
        ...
        var metaState = react.useState({ hasOverrides: false, revision: undefined });
        ...
        // Load the effective settings (host: thin proxy over ctx.settings —
        // schema defaults + patch-row base + user layer) and re-poll so an
        // open card reflects external edits while dsh web runs. In-progress
        // edits are not clobbered by the poll.
        react.useEffect(function () {
          var cancelled = false;
          function doFetch() {
            fetch('/deepseek-balance/settings', { cache: 'no-store' })
              ...
              .then(function (data) {
                if (cancelled || !data || typeof data !== 'object' || data.error) return;
                setMeta({ hasOverrides: !!data.hasOverrides, revision: data.revision });
                if (!dirtyRef.current) {
                  setFields({
        ...
        // Reflect a save/reset (or conflict re-read) response immediately;
        // the sidebar readout picks the change up on its 60 s poll.
        ...
        // A 409 means the namespace moved since the card's last read (another
        // client or an external settings.yaml edit): re-read the latest
        // values, show them, and tell the user.
        function onConflict() {
          fetch('/deepseek-balance/settings', { cache: 'no-store' })
            .then(function (res) { return res.json(); })
            .then(function (data) {
              if (data && typeof data === 'object' && !data.error) applySettings(data);
              setSave({ status: 'error', message: t('conflictStatus') });
            })
            .catch(function () {
              setSave({ status: 'error', message: t('conflictStatus') });
            });
        ...
        function onSave() {
          // Local pre-validation: thresholds must be finite, non-negative
          // numbers; displayCurrency must be one of the allowed values.
          var cny = Number(fields.warningThresholdCny);
          var usd = Number(fields.warningThresholdUsd);
          var validDisplay = fields.displayCurrency === 'cny' || fields.displayCurrency === 'usd' || fields.displayCurrency === 'both';
          if (!validDisplay || !Number.isFinite(cny) || cny < 0 || !Number.isFinite(usd) || usd < 0) {
          ...
          fetch('/deepseek-balance/settings', {
            ...
            body: JSON.stringify({
              ...
              warningThresholdCny: cny,
              warningThresholdUsd: usd,
        ...
        function onReset() {
          ...
          fetch('/deepseek-balance/settings', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ reset: true, revision: meta.revision }),
          })
            .then(function (res) { return res.json().then(function (data) { return { ok: res.ok, status: res.status, data: data }; }); })
            .then(function (result) {
              if (result.status === 409) {
      ...
      slots.inject('sidebar.footer.action', function () {
        return slots.register(
          ...
          function (props) {
            ...
            return react.createElement(BalanceDisplay, { t: t });
          }
        );
      });

      // Settings → Plugins → "Plugin configuration" tab card. The tab only
      // dispatches cards whose key is a settings namespace the Host serves;
      // the host half registers the `deepseek-balance` namespace (keyed slot
      // pairing), and this card edits settings through the host's thin-proxy
      // HTTP routes over ctx.settings.
