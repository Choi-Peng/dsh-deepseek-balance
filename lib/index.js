...
//   2. Fallback: when this plugin's row does not live in the profile patch
//      file (e.g. it is mounted from the home-level $DSH_HOME/cordis.patch.yml
//      or a custom cordis.yml), edits fall back to the legacy overrides file
//      $DSH_HOME/deepseek-balance.json, re-read on every request. A
//      successful patch write also clears those overrides so a stale legacy
//      value can never shadow the patch config.
...
/** Legacy overrides file — used only when the plugin row is not in the profile patch. */
 ...
 * profile's cordis.patch.yml. The row must already exist in that file;
 * otherwise nothing is written and the caller falls back to the legacy
 * overrides store. Throws when the patch file cannot be parsed or written.
 * @returns `{ written, rowFound, changed, mode }` — mode is 'splice',
 *   'dump', or 'noop'.
 */
function updatePatchRow(ctx, nextConfig) {
  ...
  try {
    ...
    // Unreadable/missing patch file — the row cannot live there; fall back.
  ...
  if (!found) return { written: false, rowFound: false };
      ...
      handler: async (req, res) => {
        try {
          if (req.method === 'GET' || req.method === 'HEAD') {
            ...
            const hasOverrides = rowConfigSet(config) || Object.keys(readOverrides()).length > 0;
          ...
          if (body.reset === true) {
            const patch = updatePatchRow(ctx, undefined);
            clearOverrides();
          ...
          const patch = updatePatchRow(ctx, next);
          let effective;
          let hasOverrides;
          if (patch.written) {
            clearOverrides();
            ...
            effective = { ...resolveSettings(config), ...overrides };
            hasOverrides = Object.keys(overrides).length > 0 || rowConfigSet(config);
          }
          sendJson(res, 200, { ...effective, hasOverrides });
        } catch (err) {
