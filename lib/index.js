...
// Registers an HTTP route that proxies the DeepSeek balance API and exposes
// the plugin configuration (currency display + per-currency warning thresholds)
// that lives in the profile's cordis.patch.yml under the plugin row `config`.
...
function resolveSettings(config) {
  ...
  if (config && typeof config === 'object') {
    if (config.displayCurrency === 'cny' || config.displayCurrency === 'usd' || config.displayCurrency === 'both') {
      out.displayCurrency = config.displayCurrency;
    }
    if (typeof config.warningThresholdCny === 'number' && config.warningThresholdCny >= 0) {
      out.warningThresholdCny = config.warningThresholdCny;
    }
    if (typeof config.warningThresholdUsd === 'number' && config.warningThresholdUsd >= 0) {
      out.warningThresholdUsd = config.warningThresholdUsd;
    }
  }
  return out;
}
