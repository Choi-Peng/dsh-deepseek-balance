...
/**
 * Settings namespace for the Settings → Plugins → "Plugin configuration" tab.
 * The tab only dispatches cards whose key is a settings namespace the Host
 * serves (`settings.describe()`), so this plugin registers its own namespace
 * here as the pairing key. The card itself still reads/writes config through
 * the HTTP routes below (backed by the profile's cordis.patch.yml row); the
 * namespace's resolved value tracks the composed config as its `base` layer.
 */
const BALANCE_NAMESPACE = 'deepseek-balance';

/** Schema resolving the namespace; mirrors DEFAULT_SETTINGS. */
const BALANCE_CONFIG_SCHEMA = z.object({
 ...
 * This is what makes edits in the profile's cordis.patch.yml apply without
 * relying on a fiber restart.
 */
...
/**
 * Strip undefined values and treat a missing/undefined config as equivalent to
 * DEFAULT_SETTINGS, so that "reset" (deleting the row config) is not mistaken
 * for a real change and does not trigger an unnecessary write + restart.
 */
function normalizeForCompare(config) {
  if (config == null) return { ...DEFAULT_SETTINGS };
  const out = {};
...
function deepEqual(a, b) {
  ...
  return keysA.every((key) => na[key] === nb[key]);
}
...
function buildDefaultRow(rowName, rowId, config) {
  ...
  lines.push(`      config:`);
  for (const field of SETTINGS_FIELDS) lines.push(`        ${field}: ${formatScalar(config[field])}`);
  return lines.join('\n') + '\n';
}
...
function apply(ctx, config) {
  ...
  // Register the settings namespace so the Settings → Plugins "Plugin
  // configuration" tab serves `deepseek-balance` and dispatches the card the
  // client registers under that key. The registration rides this fiber (via
  // the scoped inject context), so a config-HMR restart re-registers cleanly.
  // When the settings service is absent, the inject callback simply never
  // runs and the plugin keeps working without the tab.
  ctx.inject(['settings'], (sctx) => {
    sctx.settings.register(BALANCE_NAMESPACE, BALANCE_CONFIG_SCHEMA, {
      base: resolveSettings(config),
    });
  });
